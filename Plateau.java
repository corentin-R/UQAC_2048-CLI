...
public class Plateau {
	...
	*demande si la partie sera random ou manuelle
	*boucle de jeu:
	*-ajoute un nouceau nombre
	*-affiche le plateau
	*-demande un cou à jouer
	*/
	public void jouer()
	{
	...
	/**
	*méthode pour afficher la matrice de int
	*pb pour entiers > 9 (décalage)
	*/
	...
	/**
	*ajoute un 2 ou un 4 dans une case 
	*random du tableau
	*/
	...
	/**
	*choisit si il fau tplacer un 4 ou un 2
	*(1/5 -> 4, 4/5 -> 2)
	*@return 2 ou 4
	*/
	...
	*/
	...
	/**
	*demande une input,
	*decale tous les nombres, les fusionnent si il faut
	*puis redécale pour boucher les trous si besoin
	...
	*/
	...
	/**
	*décale tous les nombres d'une ligne y vers la droie
	*on commence de la case la plus à droite (x=length-1)
	*si la case testee est == 0 on passe à la case de gauche
	*sinon si la case à droite existe et est == 0 
	*alors on échange la case testee et la case de droite
	*puis on passe à la case de gauche suivante (méthode récursive)
	*@args coordonnees de la case a tester
	*/
	private void deplacerLigneDroite(int x,int y)
	{
		//test si on est toujours dans le tableau
		if (x<0 || x>length-1 || y<0 || y>length-1) return;
	...
	/**
	*permet de fusionner les cases d'une ligne quand necessaire
	*on part de la case la plus a droite de la ligne
	*si la case à droite existe et est egale à la case testee
	*alors on fusionne et la case testee devient =0
	*puis on passe à la case de gauche suivante (méthode récursive)
	*@args coordonnees de la case a tester
	*/
	...
	{
			//test si on est toujours dans le tableau
		if (x<0 || x>length-1 || y<0 || y>length-1 || board[x][y]==0)
			return;
